 ...
 */
void *descripteur_de_texte(FILE *src, PILE *p, Table_Index *table_index)
{
 ...
 */
void fabrique_a_descripteur(char *path_to_xml, PILE_descripteur_texte *pile_desc, Table_Index *table_index)
{
...
int Descripteur_texte_dossier(char *nom_dossier, PILE_descripteur_texte *pile_desc, Table_Index *table_index)
{
    ...
    if (f)
    {
        ...
        while (fscanf(f, "%s", nom_fichier) != EOF)
        {
            ...
            fabrique_a_descripteur(path_to_xml, pile_desc, table_index);
        }
        fclose(f);
        remove("tmp");
        remove("tmp1");
...
{
    fabrique_a_descripteur(nom_fichier, pile_desc, table_index);
    remove("tmp");
    remove("tmp1");
}
